...
@RequestScoped
@FacesValidator("com.java.project.PhoneVerification")
public class PhoneVerification implements Validator, Serializable{
	...
	public void validate(FacesContext context, UIComponent comp, Object value) throws ValidatorException {
		...
		try {
	    	...
	    	cr.add(Restrictions.eq("phoneNo", value));
	    	cr.setProjection(Projections.rowCount());
	    	Long count = (Long) cr.uniqueResult();
	    	if(count == 1)
	    	{
	    		FacesMessage msg = new FacesMessage("An account is already registered with this number", uniqueColumn);
