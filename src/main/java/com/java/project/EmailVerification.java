...
@RequestScoped
@FacesValidator("com.java.project.EmailVerification")
public class EmailVerification implements Validator{
	...
	public void validate(FacesContext context, UIComponent comp, Object value) throws ValidatorException {
		...
		try {
	    	...
	    	cr.add(Restrictions.eq("providerEmailId", value));
	    	cr.setProjection(Projections.rowCount());
	    	Long count = (Long) cr.uniqueResult();
	    	if(count == 1)
	    	{
	    		FacesMessage msg = new FacesMessage("An account is already registered with this email Id", uniqueColumn);
